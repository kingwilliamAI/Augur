...
**Сжечь купленное.** Отправка на «мёртвый» адрес ничего не сжигает: токены просто лежат по адресу,
от которого нет ключа, а `totalSupply` остаётся прежним. У этого токена перевод на нулевой адрес
вообще откатывается — на это все и натыкаются. Настоящее сжигание это вызов `burn` у самого токена,
и он уменьшает supply, который потом любой может прочитать с контракта:
...
По умолчанию сжигается весь баланс кошелька выкупа; часть — через `--amount 1000000`. Команда
сначала симулирует вызов, печатает, какой supply останется, и без `--send` не делает ничего. Это
самая необратимая операция во всём проекте: вернуть сожжённое нельзя никак.
...
3. Убедиться, что вотчер записал смену: `npm run stats` показывает счётчик `feechg`, а карточка
   токена — строку про смену получателя. Обновлять сам лаунч не нужно: в нём хранится адрес из
   calldata лаунча, и `enrich-window` перечитает то же самое. Леджер и `payout` берут текущего
   получателя из этих событий.
4. `npm run fees` — прочитает условия прямо из контракта, запишет их в `meta` и подтянет выплаты.
   Старый кошелёк он продолжит читать тоже, так что история до смены не пропадает.
