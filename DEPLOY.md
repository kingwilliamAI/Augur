...
Обозначения: `[локально]` — на девбоксе, `[сервер]` — на новой машине.
...
Пока обе строки ниже стоят в нуле, платная половина выключена и все читатели получают всё. Это
рабочий режим до того, как решены пороги: механика крутится на живых людях, а число, которое пока
нечем обосновать, не выдумывается.
...
FREE_DELAY_SEC=60
FREE_MIN_SCORE=10
TIER_COOLDOWN_SEC=604800
TIER_RECHECK_SEC=3600
RATE_LIMIT_KEYED=2000
SITE_URL=https://getaugur.xyz
```

`TIER1_TOKENS` и `TIER2_TOKENS` — в целых токенах, не в wei. Сапплай $AUGUR ровно 1 000 000 000, так
что 100000 это 0.01%, а 1000000 — 0.1%. Ставить их можно на живой системе: бот перечитывает балансы
каждый `TIER_RECHECK_SEC` и сам скажет каждому, у кого тир поменялся.

`FREE_DELAY_SEC` и `FREE_MIN_SCORE` — то, чем бесплатный бот отличается от платного: задержка от
момента записи скора и нижний порог. `TIER_COOLDOWN_SEC` — через сколько тир возвращается после
продажи; на нём держится то, что тир нельзя одолжить на одну минуту.

Балансы читаются с `RPC_STATE_URL`. Неудачное чтение ничего не меняет: упавший эндпоинт не должен
снимать тир со всех сразу и отправлять их на недельный кулдаун.

`SITE_URL` — на неё бот шлёт ссылку из `/link`. Если она указывает не туда, кнопка в боте ведёт в никуда;
на локальном запуске ставить `http://localhost:4663`.

**Проверить после рестарта бота:** написать ему `/status` — там должна появиться строка про тир, и
`/link` — по ссылке должна открыться страница с кнопкой connect. Caddy отдаёт `/link` тем же
проксированием, отдельного правила не нужно: доска сама отвечает на этот путь.

### Два новых процесса

Ни один из них не обязателен: без них доска и бот работают как раньше, просто карточка не показывает
происхождение кошелька, а лидерборд трейдеров пустой.

`npm run tail` — читает тела блоков и ищет нативные переводы, у которых нет логов. Идёт только
вперёд: свежесть кошелька это вопрос о состоянии на прошлом блоке, а публичные эндпоинты отвечают
на него максимум минут десять назад. Бэкфила нет и быть не может. Цена: около 5% ядра и ~6 ГБ в
сутки входящего (gzip; в разжатом виде это ~40 ГБ, но по проводу идёт сжатое).

`npm run traders` — разовый или периодический проход по логам кривых, сворачивает их в позиции.
Сырьё не хранится: 1.13M сделок в сутки это полгигабайта в день, а нужны из них три числа на
кошелёк и токен. Идёт по тому же logs-эндпоинту, что и вотчер, поэтому гонять его лучше не в час пик.

Обе штуки читают ту же базу, что доска и бот, так что отдельных юнитов достаточно скопировать с
`ponscan-watch`, поменяв `ExecStart`.
...
## 10. Разнесение комиссии

Два пути к одному и тому же: софтом сегодня и контрактом потом. Софт проще и обратим, контракт
убирает необходимость вам верить. Оба лежат в репозитории, и страница `/#/fees` честно пишет, кто из
двух заплатил каждую строчку.

### 10a. Софтом: `npm run payout`

Это единственная команда во всём проекте, которая подписывает транзакции. Поэтому она отдельная,
запускается руками или таймером на той машине, где лежит кошелёк комиссии, и до неё не дотягивается
ни доска, ни бот, ни сканер.
...
PAYOUT_RESERVE_ETH=0.01
PAYOUT_MIN_ETH=0.02
PAYOUT_PRIVATE_KEY=0x…
```

Доли в bps и должны складываться ровно в 10000, иначе команда откажется работать. Все три нуля по
умолчанию — это сознательно: пока доли не выбраны, разносить нечего.

**Про ключ.** Он лежит в `.env` на сервере в открытом виде, и это горячий ключ: на этом кошельке
должна быть только комиссия и ничего больше. `.env` уже в `.gitignore`; проверьте, что он не уезжает
в бэкапы, и держите на файле права `600`:

```bash
chmod 600 /home/augur/augur/.env
```

**Прогон.** Сначала всегда без ключа или без `--send` — команда печатает план и ничего не трогает:
...
Первый боевой запуск разумно ограничить потолком, чтобы посмотреть на цепи, что всё уходит куда надо:

```bash
npm run payout -- --max 0.1 --send
```

Если комиссия приходит не в нативной валюте, то же самое с адресом токена: `npm run payout -- --asset 0x… --send`.

Отправка идёт по одной транзакции с ожиданием квитанции: при ошибке команда останавливается, остаток
остаётся на кошельке и уйдёт следующим прогоном. Каждый перевод пишется в базу только после того, как
он замайнен, так что лента показывает то, что произошло, а не то, что задумывалось.

**Раз в три часа.** Два способа, любой годится. Таймер systemd переживает ребут и не держит процесс:

```ini
# /etc/systemd/system/augur-payout.service
[Unit]
Description=augur fee payout
After=network-online.target
...
User=augur
WorkingDirectory=/home/augur/augur
ExecStart=/usr/bin/node --no-warnings src/cli/payout.ts --send
```

```ini
# /etc/systemd/system/augur-payout.timer
[Unit]
Description=split the fee wallet every three hours
...
```bash
sudo systemctl daemon-reload
sudo systemctl enable --now augur-payout.timer
```

Либо одним процессом рядом с вотчером, если удобнее держать всё под одним супервизором:

```bash
npm run payout -- --every 3h --send
```

Упавший прогон не заканчивает процесс: то, что не ушло, остаётся на кошельке и уйдёт следующим разом.

Смотреть, что он делал: `journalctl -u augur-payout -n 50` и страница `/#/fees`.

---

### 10b. Выкуп: `npm run buyback`

Вторая команда, которая подписывает, и единственная, которая торгует. Она тратит кошелёк выкупа —
тот самый, на который `payout` отправляет свою долю, — покупая токен в пуле, куда pons его вывел.
...
BUYBACK_RESERVE_ETH=0.005
BUYBACK_MIN_ETH=0.01
```

Роутер и пул уже прописаны по умолчанию, и это не догадка: и то и другое снято с реальной
транзакции покупки этого токена. У токена есть ещё шесть чужих пулов с комиссией до 80%, поэтому
ключ пула прибит гвоздями, а не ищется на лету.

**Откуда берётся цена.** Ни оракула, ни квотера: команда просит цепь симулировать своп с
минимальным выходом и поднимает планку, пока симуляция не откажет. Это и есть то, что пул реально
платит, с учётом комиссии хука. На цепь уходит эта цифра минус проскальзывание — если кто-то влезет
сэндвичем, транзакция откатится, а не исполнится по плохой цене. Фил ровно по нижней планке
помечается в ленте отдельно: так выглядит опережение.
...
```bash
npm run buyback -- --max 0.02 --send
```

Дальше по расписанию, с размазанным интервалом (регулярная покупка предсказуемого размера читается
с цепи за вечер, поэтому по умолчанию интервал гуляет на ±20 минут):

```bash
npm run buyback -- --every 3h --send
```

Купленные токены остаются на кошельке выкупа. Сжигать их, запирать или держать — отдельное решение и
отдельная транзакция, инструмент об этом мнения не имеет.

---

### 10c. Контрактом: `FeeSplitter.sol` (когда будете готовы)

Сейчас креаторская комиссия $AUGUR приходит на обычный кошелёк, и договорённость о том, как она
делится, держится на слове. `contracts/FeeSplitter.sol` заменяет слово контрактом: он делит каждую
выплату на три адреса в долях, зашитых при деплое. У него нет владельца, нет сеттеров, нет паузы и
нет вывода остатка, так что схему можно прочитать с цепи, а не принять на веру. Поменять её можно
только новым деплоем и сменой получателя в лаунче, и это транзакция, которую видно всем.

**Что решить до деплоя.** Три адреса (сервер, выплаты холдерам, выкуп) и три доли в bps, дающие в
сумме ровно 10000. Например 4000 / 4000 / 2000. Долю потом не изменить: это и есть смысл контракта.

**Деплой.** Компилятора в репозитории нет: одна рантайм-зависимость это принцип, а не случайность.
Собрать и задеплоить проще всего в Remix (`Solidity 0.8.24`, EVM Cancun, оптимизатор включён) с
кошелька, подключённого к Robinhood Chain, либо любым `forge create` со своей машины. Конструктор
принимает шесть аргументов ровно в этом порядке:

```
server, holders, buyback, serverBps, holdersBps, buybackBps
```

**После деплоя, по шагам:**

1. Записать адрес в `.env` на сервере: `FEE_SPLITTER=0x…`
2. Перевести получателя комиссии лаунча на этот адрес. Это делается с кошелька создателя, через
   pons; фактория логирует `CreatorFeeRecipientUpdated`, и доска эту запись видит.
3. `npm run enrich-window` (или дождаться ночного прогона), чтобы в базе обновился
   `creator_fee_recipient` у токена.
4. `npm run fees` — прочитает условия прямо из контракта, запишет их в `meta` и подтянет выплаты.
5. Открыть `/#/fees`: в шапке должно стоять «In force», а доли и адрес должны совпасть с тем, что вы
   деплоили. Если стоит «Deployed, not yet in force» — второй шаг не доехал.

**Как деньги оттуда уходят.** Комиссия копится в эскроу pons и приходит на контракт, когда её
клеймят; дальше `release()` делит то, что лежит на контракте, и его может вызвать кто угодно, потому
что уйти деньги могут только на три зашитых адреса. Если лаунч котирован не в нативной валюте, то же
самое делает `releaseToken(<адрес токена>)`. Остаток от деления (пара wei) уходит холдерам.

**Таймер для чтения ленты выплат** ставится тем же способом, что и перекалибровка в разделе 7:

```ini
# /etc/systemd/system/augur-fees.service
[Unit]
Description=augur fees ledger
After=network-online.target
...
User=augur
WorkingDirectory=/home/augur/augur
ExecStart=/usr/bin/node --no-warnings src/cli/fees.ts
```

```ini
# /etc/systemd/system/augur-fees.timer
[Unit]
Description=read the fee ledger hourly
...
```bash
sudo systemctl daemon-reload
sudo systemctl enable --now augur-fees.timer
```

**Пока контракт не задеплоен**, страница `/#/fees` честно говорит, что комиссия падает на кошелёк, и
показывает его ленту. Это состояние не ошибка и не заглушка: это то, как оно есть сегодня.
